...
void loop()
{int tVariable = i;
Serial.println(tVariable);
  delay(1);

if(!(digitalRead(LS)) && !(digitalRead(RS)) && !(digitalRead(CS))) // Move Forward on line 111 all black T
{if(i%2==0)   //turn left if even
...
if( digitalRead(LS) && digitalRead(RS) && digitalRead(CS)) // BACK IF IT GOES OUT OF RANGE 000 all white
{i++;
delay(2000);
digitalWrite(LM1, LOW);
digitalWrite(LM2, LOW);
